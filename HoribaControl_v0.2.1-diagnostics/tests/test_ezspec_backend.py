...
async def test_discovery_error_is_not_masked_by_icl_shutdown_error():
    backend = EzSpecBackend(
        {"poll_interval_s": 0, "discovery_timeout_s": 1},
        {"monochromator_index": 0, "camera_index": 0},
        Logger(),
        manager_factory=BrokenDiscoveryManager,
    )
    try:
        await backend.connect()
    except RuntimeError as exc:
        assert "réponse sans clé 'devices'" in str(exc)
    else:
        raise AssertionError("Une erreur de découverte explicite était attendue.")
