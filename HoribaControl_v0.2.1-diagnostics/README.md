...
## Dépannage : `KeyError: 'devices'`

Cette erreur vient de la découverte CCD du SDK : ICL n’a pas renvoyé la structure
attendue contenant une liste `devices`.

Procédure recommandée :

1. utiliser Python 3.12 ou 3.13, pas Python 3.14 ;
2. fermer LabSpec/SynerJY et toute autre application HORIBA ;
3. terminer les processus `ICL.exe` restants dans le Gestionnaire des tâches ;
4. vérifier que la Syncerity est alimentée et reconnue dans SynerJY/LabSpec ;
5. vérifier l’activation/licence EzSpec ;
6. relancer `horibacontrol diagnose`.

La v0.2.1 conserve désormais cette erreur initiale, même si ICL refuse ensuite
de s’arrêter.
