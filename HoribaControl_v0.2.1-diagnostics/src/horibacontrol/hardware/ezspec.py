...
class EzSpecBackend(InstrumentBackend):
    ...
    async def connect(self) -> dict:
        ...
        try:
            ...
            try:
                await self._safe_manager_stop(suppress_errors=True)
            finally:
                self.monochromator = None
                self.camera = None
                self.connected = False

            if isinstance(original_error, KeyError) and original_error.args == ("devices",):
                raise RuntimeError(
                    "La découverte CCD d’ICL a renvoyé une réponse sans clé 'devices'. "
                    "Cela indique généralement qu’ICL a renvoyé un message d’erreur "
                    "au lieu d’une liste de caméras. Vérifiez que la Syncerity est "
                    "allumée et visible dans SynerJY/LabSpec, que la licence EzSpec "
                    "est active, qu’aucune autre application ne contrôle le matériel, "
                    "puis réessayez avec Python 3.12 ou 3.13."
                ) from original_error
            raise
    ...
    async def _safe_manager_stop(self, suppress_errors: bool = False) -> None:
        ...
        try:
            ...
            if suppress_errors:
                self.logger.debug(
                    "Échec de l’arrêt d’ICL pendant le nettoyage ; "
                    "l’erreur initiale est conservée.",
                    exc_info=True,
                )
                return
            raise
